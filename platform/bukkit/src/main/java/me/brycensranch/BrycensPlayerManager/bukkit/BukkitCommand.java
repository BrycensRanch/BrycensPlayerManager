...
public final class BukkitCommand implements CommandExecutor, TabCompleter {
  ...
  public boolean onCommand(final @NonNull CommandSender sender, final @NonNull Command command, final @NonNull String label, final @NonNull String[] args) {
    ...
    if (!sender.hasPermission("bpm.admin")) {
  ...
  private void onInvalidUse(final @NonNull Audience audience) {
    audience.sendMessage(text("Invalid command usage. Use '/bpm help' for a list of command provided by BrycensPlayerManager.", RED)
      .hoverEvent(text("Click to execute '/bpm help'"))
      .clickEvent(runCommand("/bpm help")));
  }
  ...
  public List<String> onTabComplete(final @NonNull CommandSender sender, final @NonNull Command command, final @NonNull String alias, final @NonNull String[] args) {
    if (args.length < 2 && sender.hasPermission("bpm.admin")) {
      return COMMANDS;
    }
